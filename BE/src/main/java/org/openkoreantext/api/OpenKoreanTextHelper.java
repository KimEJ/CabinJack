...
public class OpenKoreanTextHelper {
	...
	public List<KoreanPhraseExtractor.KoreanPhrase> extractPhrase(String text) {
		return extractPhrasesJson(text);
	}
	
	private List<KoreanPhraseExtractor.KoreanPhrase> extractPhrasesJson(String text) {
		CharSequence normalized = OpenKoreanTextProcessorJava.normalize(text);
		Seq<KoreanTokenizer.KoreanToken> tokens = OpenKoreanTextProcessorJava.tokenize(normalized);
